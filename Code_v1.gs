// --- Configuration ---
const FOLDER_ID = '1f7esiYvQj7MElui8SFYnJ8Vc7GmBlQ5G'; // <-- Replace with your Input Drive folder ID
const ARCHIVE_FOLDER_ID = '1byCIXZlCuVy4vepM0buxyC7u008NcMFk'; // <--- NEW: Replace with your Archive Drive folder ID
...
function checkNewImagesAndQC() {
  const inputFolder = DriveApp.getFolderById(FOLDER_ID);
  const archiveFolder = DriveApp.getFolderById(ARCHIVE_FOLDER_ID); // <--- NEW: Get archive folder
  ...
  Logger.log(`Checking for files modified after: ${new Date(checkFromTimestamp).toISOString()} in folder: ${inputFolder.getName()}`);

  // IMPORTANT: Ensure this search query is correct for deployment.
  // It should be modifiedDate > for newly added files.
  // const files = inputFolder.searchFiles(
  ...
  let processedFilesCount = 0; // Renamed for clarity
  let failedFilesCount = 0;    // <--- NEW: Counter for failed files
  ...
  while (files.hasNext()) {
    ...
    try {
      ...
      // If processImageWithGemini completes without throwing an error, move the file
      file.moveTo(archiveFolder); // <--- NEW: Move to archive folder on successful processing
      Logger.log(`Successfully processed and moved ${file.getName()} to archive folder.`);
      processedFilesCount++;
    } catch (e) {
      Logger.log(`Error processing file ${file.getName()}: ${e}\nStack: ${e.stack}`);
      sendAlert(`QC Script Error Processing ${file.getName()}`, `File: ${file.getName()}\nURL: ${file.getUrl()}\nError: ${e}\nStack: ${e.stack}`);
      failedFilesCount++; // <--- NEW: Increment failed counter
      // File remains in the input folder for manual inspection if processing fails
    }
  }
  ...
  Logger.log(`Checked folder: ${inputFolder.getName()}`);
  Logger.log(`Processed ${processedFilesCount} files successfully.`);
  Logger.log(`Failed to process ${failedFilesCount} files.`);
  Logger.log(`Last check updated to: ${new Date(nowTimestamp)}`);
  Logger.log(`-------------------`);
}
...
function processImageWithGemini(file) {
  ...
  try {
    ...
    if (jsonResponse.candidates && jsonResponse.candidates.length > 0 &&
        ...
        throw new Error(`Gemini request blocked: ${reason}`); // Throw error to prevent deletion
    } else {
        ...
        throw new Error(`Unexpected Gemini response structure.`); // Throw error to prevent deletion
    }
  } catch (e) {
    ...
    throw e; // Re-throw to ensure the file is not deleted
  }
}
...
function performQC(prescription, measurement, fileName, fileUrl) {
  ...
  if (mismatches.length > 0) {
    ...
    // Optional: Log success or take other action (e.g., move file to a "Passed" folder within archive)
  }
}
...
// --- Setup Trigger (ensure this is run once to set up the daily trigger) ---
function setupDailyTrigger() {
  // Ensure 'processDailyQCPictures' function name matches your main function if you rename it
  ScriptApp.newTrigger('checkNewImagesAndQC')
      .timeBased()
      .atHour(17) // e.g., 5 PM (17:00)
      .everyDays(1)
      .create();
  Logger.log('Daily trigger set for 5 PM to run checkNewImagesAndQC.');
}

// --- Initial setup function (run this ONCE after deploying the script) ---
function initialSetup() {
    // IMPORTANT: Make sure you've created these folders in Google Drive manually
    // and replaced the placeholder IDs above with their actual IDs.
    // Also, ensure your GEMINI_API_KEY is set in Script Properties.
    // You can set properties by going to Project Settings -> Script Properties -> Add new property.

    setupDailyTrigger();
    Logger.log('Initial setup complete. Daily trigger created.');
}
